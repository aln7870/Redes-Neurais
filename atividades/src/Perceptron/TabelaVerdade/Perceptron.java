    ...
    public void treinar(int[][] entradas, int[] saidas){
        for (int i = 0; i < epocas; i++){
            ...
            for (int j = 0; j < entradas.length; j++){
                ...
                int saidaPredita = prever(x1, x2); // calcula com os pesos atuais
                int erro = saidaEsperada - saidaPredita; // diferença entre o esperado e o obtido
                erroTotal += Math.abs(erro); // soma os erros para ver se ainda precisa continuar treinando

                //Exibindo os pesos
                System.out.printf("Antes da atualização: Pesos = [%.0f], [%.0f], [%.0f] \n", pesos[0], pesos[1], pesos[2]);
                ...
                //Exibindo os pesos
                System.out.printf("Depois da atualização: Pesos = [%.0f], [%.0f], [%.0f] \n", pesos[0], pesos[1], pesos[2]);
            }
